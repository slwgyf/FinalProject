...
void handleBets (Dealer dealer, Player player1, int bet)
{
	...
	{
		bank = bank + bet;
		cout<< "Congrats, "<< player1.getName()<< " you won. Your bank is now $"<< bank<< endl;
		
	}
	else if (sum > 21)
	{
		bank = bank - bet;
		cout<< "Sorry, "<< player1.getName()<< " you lost. Your bank is now $"<< bank<< endl;
...
int errorCheck (int bound1, int bound2, int input)
{

	while (cin.fail())
	{
		cout<< "Please enter an integer.  Try again:" <<  endl<<">>";//WHY IS THIS REPEATING FOR EVERY CHARACTER
		cin.clear();
		cin.ignore();
		cin>> input;
	...
	while (input < bound1 || input > bound2)
	{
		cout<< "Invalid input. Max:"<< bound2<<" Min:"<<bound1<< endl<<">>";
		cin >> input;
	}
	return input;
...
int main (void)
{
	...
	if (fileStatus == 0)//no saved games
	{
		...
		while (cin.fail())
		{
			cout<< "Please enter an integer.  Enter number of players: (Maximum:4, Minimum: 1)" << endl;
			cin >>numPlayers;
		}
	...
	else//there are saved users
	{
		...
		cout<< "Enter number of players: (Maximum:4, Minimum: 1)" << endl;
		cin>> numPlayers;
		numPlayers = errorCheck(1, 4, numPlayers);
		//for reg error checking uncomment these^^
				/*	while (cin.fail())
					{
						cout<< "Please enter an integer.  Enter number of players: (Maximum:4, Minimum: 1)" << endl;
						cin.clear();
...
{	
	...
	{
		cout<< table[j].getName()<< " it is your turn.  How much would you like to bet?"<< endl<< ">>";
		cin >> bets[j];
	...
	hand.display();
	sum = hand.getSum();
	cout << sum<< endl;//just for error check purposes
	if (sum > 21)
	{
...
free (bets);
updateFile("-1", 500);//one last update to save all changes and check for zeroes
	return 0;
}

//Ace needs to be handled->11 or 1
//if dealer and player busts what happens
